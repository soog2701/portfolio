...
const path = require("path"),
    getPageNamespaces = require("../_helpers/getPageNamespaces").default;
...
function getLangs () {
    return allLanguages.filter((lng) => defaultLangRedirect === "lang-path" || lng !== defaultLanguage);
}
    ...
    readDirR(currentPagesDir).forEach(async (page) => {
      ...
      // Clear index folder case
          replace(/\/index$/, "") || "/",
            ...
            namespaces = await getPageNamespaces({pages}, pageId);
...
function hasSpecialMethod (data, name) {
    return data.match(new RegExp(`export (const|var|let|async function|function) ${name}`));
}
...
function getPageTemplate (prefix, page, lang, namespaces) {
    const clearCommentsRgx = /\/\*[\s\S]*?\*\/|\/\/.*/g,
        ...
        isGetStaticProps = hasSpecialMethod(pageData, "getStaticProps"),
        isGetStaticPaths = hasSpecialMethod(pageData, "getStaticPaths"),
        isGetServerSideProps = hasSpecialMethod(pageData, "getServerSideProps"),
        hasSomeSpecialMethod =
...
${namespaces.
        map((ns, i) => `import ns${i} from '${prefix}/${localesPath}/${lang}/${ns}.json'`).
        join("\n")}
...
function buildPageLocale ({prefix, pagePath, namespaces, lang, path}) {
    const finalPath = pagePath.replace(currentPagesDir, path),
        template = getPageTemplate(prefix, pagePath, lang, namespaces),
        [filename] = finalPath.split("/").reverse(),
        dirs = finalPath.replace(`/${filename}`, "");
